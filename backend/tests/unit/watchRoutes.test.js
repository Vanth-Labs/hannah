...
describe('lo que entra y lo que sale', () => {
    ...
    test('/api/v1/health gana el contador de vigilancias, y sigue siendo abierto', async () => {
        ...
        // `pending` son disparos ocurridos y todavía no contados en voz alta y `stalled` los que
        // además se dejaron de intentar: con el buzón vacío son 0, y es el único lugar donde un
        // huérfano —que por plan §10 no se narra nunca— se ve sin leer el disco.
        expect(body.watches).toEqual({ armed: 1, degraded: 0, blind: 1, suspended: 0, pending: 0, stalled: 0, lastSampleAt: 9000 });
    });
});
